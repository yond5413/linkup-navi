...
class ContentStore:
    ...
    async def add_pdf(self, content: str, file_path: str, title: Optional[str] = None, session_id: Optional[str] = None) -> ContentItem:
        if title is None:
            title = Path(file_path).name
        item = await self._create_content_item(
            source_type=SourceType.PDF, title=title, content=content,
            session_id=session_id, file_path=file_path)
        return item
    
    async def add_email(self, subject: str, sender: str, body: str, recipients: Optional[List[str]] = None,
                       thread_id: Optional[str] = None, timestamp: Optional[datetime] = None,
                       session_id: Optional[str] = None) -> ContentItem:
        item = await self._create_content_item(
            source_type=SourceType.EMAIL, title=subject, content=body,
            session_id=session_id, sender=sender, recipients=recipients or [],
            thread_id=thread_id, timestamp=timestamp)
        return item
    
    async def add_message(self, content: str, sender: str, title: Optional[str] = None,
                         recipients: Optional[List[str]] = None, thread_id: Optional[str] = None,
                         timestamp: Optional[datetime] = None, session_id: Optional[str] = None) -> ContentItem:
        item = await self._create_content_item(
            source_type=SourceType.MESSAGE, title=title or "Message", content=content,
            session_id=session_id, sender=sender, recipients=recipients or [],
            thread_id=thread_id, timestamp=timestamp)
        return item
    
    async def add_research(self, query: str, result: Dict[str, Any], title: Optional[str] = None,
                          thread_id: Optional[str] = None, session_id: Optional[str] = None) -> ContentItem:
        content = json.dumps(result, indent=2)
        item = await self._create_content_item(
            source_type=SourceType.RESEARCH, title=title or f"Research: {query}",
            content=content, session_id=session_id, thread_id=thread_id, metadata={"query": query})
        return item
    
    async def add_note(self, title: str, content: str, thread_id: Optional[str] = None,
                      session_id: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> ContentItem:
        item = await self._create_content_item(
            source_type=SourceType.NOTE, title=title, content=content,
            session_id=session_id, thread_id=thread_id, metadata=metadata or {})
        return item
    
    async def _create_content_item(self, source_type: SourceType, title: str, content: str,
                                  session_id: Optional[str] = None, file_path: Optional[str] = None,
                                  sender: Optional[str] = None, recipients: Optional[List[str]] = None,
                                  timestamp: Optional[datetime] = None, thread_id: Optional[str] = None,
                                  metadata: Optional[Dict[str, Any]] = None) -> ContentItem:
        item_id = str(uuid.uuid4())
        now = datetime.now(timezone.utc)
        
        item = ContentItem(
            id=item_id, session_id=session_id, source_type=source_type,
            title=title, content=content, metadata=metadata or {},
            file_path=file_path, sender=sender, recipients=recipients,
            timestamp=timestamp or now, thread_id=thread_id)
    ...
    async def _persist_item(self, item: ContentItem):
        ...
        await db.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            item.id, item.session_id, item.source_type.value, item.title,
            item.content, json.dumps(item.metadata), item.file_path, item.sender,
            json.dumps(item.recipients) if item.recipients else None,
            item.timestamp.isoformat() if item.timestamp else None,
            item.thread_id, item.embedded_at.isoformat() if item.embedded_at else None,
            item.embedding_id
        ))
